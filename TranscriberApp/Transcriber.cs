...
namespace TranscriberApp
{
    public class Transcriber : ITranscriber
    {
        ...
        public string TranscribeAudioFile(string audioFilePath, Dictionary<string, object> options)
        {
            ...
            string arguments = $"--model {modelPath} --language {language} ";
            ...
            using (Process process = new Process())
            {
                ...
                // Przechowaj wynik transkrypcji
                string transcriptionResult = "";
                
                // Obsługa wyjścia standardowego
                process.OutputDataReceived += (sender, e) =>
                {
                    if (!string.IsNullOrEmpty(e.Data))
                    {
                        transcriptionResult += e.Data + Environment.NewLine;
                    }
                };
